...
func TestSecretServiceIntegration(t *testing.T) {
	...
	t.Run("discovery uses metadata only", func(t *testing.T) {
		store, err := newSecretServiceStore(context.Background())
		if err != nil {
			t.Fatal(err)
		}
		refs, err := store.Search(context.Background(), credentialQuery{Collection: "default", Text: "integration", AllowInteraction: true})
		if err != nil || len(refs) != 1 || refs[0].Label != "integration" || refs[0].Collection != "Integration" {
			t.Fatalf("credential discovery = %#v, %v", refs, err)
		}
	...
	t.Run("prompt retrieval and cleanup", func(t *testing.T) {
		...
		secret, err := store.Secret(context.Background(), refs[0], credentialReadOptions{AllowInteraction: true})
		if err != nil || string(secret) != "synthetic-integration-password" {
			t.Fatalf("secret retrieval failed: %v", err)
		}
		clearBytes(secret)
		if err := store.Close(); err != nil {
			t.Fatal(err)
	...
	t.Run("provider error is preserved", func(t *testing.T) {
		...
		_, err = store.Secret(context.Background(), refs[0], credentialReadOptions{AllowInteraction: true})
	...
	t.Run("unavailable service is classified", func(t *testing.T) {
		if err := provider.releaseName(); err != nil {
			t.Fatal(err)
		}
		releasedNames := integrationBusNames(t, provider.conn)
		if _, err := newSecretServiceStore(context.Background()); err == nil {
